...
/*   Updated: 2026/07/29 15:12:07 by zatais           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Config::assignServerDirective(ServerConfig& server, std::vector<std::string>& tokens, size_t& idx) {
  ...
  {
    long cmbs = atol(tokens[idx++].c_str());
    if (cmbs < 0)
      throw std::runtime_error("config: negatif value for client_max_body_size");
    server.client_max_body_size = cmbs;
  }
  else
...
void Config::assignLocationDirective(Location& loc, std::vector<std::string>& tokens, size_t& idx) {
  ...
  if (key == "root")
    ...
    if (values.size() == 1) {
      ...
      loc.redirect.enabled = true;
      loc.redirect.code = toInt(values[0], key, 300, 399);
