...
void ProcessRequest::match_location(ServerConfig &server) {
  ...
  // added
  std::string path_after_location =
      request->getPath().substr(target_location.path.length());
      ...
      if (access(resource_path.c_str(), F_OK | X_OK))
        ...
        if (pos != resource_path.length() - 1) {
          if (request->getRequestLine().Method == "GET") {
            ...
            redirect_url = "http://" + request->getHost() + ":" +
                           port_ss.str() + request->getPath() + "/";
...
std::string ProcessRequest::getExtension() const { return (extension); }

int ProcessRequest::getStatusCode() { return (status_code); }

std::string ProcessRequest::getResourcePath() const { return (resource_path); }

std::string ProcessRequest::getIndexFile() const { return (Index_file); }

std::string ProcessRequest::getCgiPath() const { return (cgi_path); }

Location ProcessRequest::getLocation() const { return (target_location); }

std::string ProcessRequest::getRedirectUrl() const { return (redirect_url); }

void ProcessRequest::setStatusCode(int code) { status_code = code; }
...
void ProcessRequest::setRedirctUrl(std::string &url) { redirect_url = url; }
